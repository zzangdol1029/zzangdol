...
class SpecialEffect(QMainWindow):
    def __init__(self):
        ...
        pictureButton=QPushButton('사진 열기',self)
        ...
        self.pickCombo.addItems(['엠보싱 저장','카툰 저장','연필 스케치 저장',
                                 '컬러 스케치 저장','유화 저장'])
    ...
    def pictureOpenFunction(self):
        fname=QFileDialog.getOpenFileName(self,'사진 열기','./')
        self.img=cv.imread(fname[0])
        if self.img is None: sys.exit('파일을 찾을 수 없습니다.')
        cv.imshow('Painting',self.img)

    def embossFunction(self):
        f=np.array([[-1,-1,0],[1,0,1],[0,1,1]])
        gray=cv.cvtColor(self.img,cv.COLOR_BGR2GRAY)
        gray16=np.int16(gray)
        self.emboss=np.uint8(np.clip(cv.filter2D(gray16,-1,f)+128,0,255))
    ...
    def sketchFunction(self):
        self.sketch_gray,self.sketch_color=cv.pencilSketch(self.img,sigma_s=60,
                                                            sigma_r=0.07,shade_factor=0.02)

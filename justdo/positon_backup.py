...
def classify_position(position):
    """직급을 주요 직급(담당, 책임, 수석)과 기타로 분류"""
    main_positions = ['담당', '책임', '수석']
    
    if position in main_positions:
        return position
    elif position and position not in ["N/A", "ERROR", "AUTH_ERROR", "HTTP_ERROR", "AUTH_REQUIRED"]:
        return "기타"
    else:
        return None
...
def create_team_charts(team_stats, all_positions):
    ...
    # 직급 분류 적용
    classified_team_stats = {}
    for team_num, team_data in team_stats.items():
        classified_team_stats[team_num] = {}
        for position, count in team_data.items():
            classified_pos = classify_position(position)
            if classified_pos:
                if classified_pos in classified_team_stats[team_num]:
                    classified_team_stats[team_num][classified_pos] += count
                else:
                    classified_team_stats[team_num][classified_pos] = count
    
    # 분류된 직급 목록
    classified_positions = ['담당', '책임', '수석', '기타']
    
    # 1. 전체 조별 비교 막대 그래프
    plt.figure(figsize=(15, 8))
    
    # 각 직급별로 조들의 데이터 준비
    teams = sorted(classified_team_stats.keys())
    x = np.arange(len(teams))
    width = 0.8 / len(classified_positions)
    
    colors = ['#FF9999', '#66B2FF', '#99FF99', '#FFCC99']  # 담당, 책임, 수석, 기타
    
    for i, position in enumerate(classified_positions):
        counts = [classified_team_stats[team].get(position, 0) for team in teams]
        plt.bar(x + i * width, counts, width, label=position, color=colors[i])
    
    plt.xlabel('조')
    plt.ylabel('인원수')
    plt.title('조별 직급 분포 (담당/책임/수석/기타)')
    plt.xticks(x + width * (len(classified_positions) - 1) / 2, [f'{team}조' for team in teams])
    plt.legend()
    ...
    total_by_position = {}
    for team_data in classified_team_stats.values():
        for position, count in team_data.items():
    ...
    counts = []
    chart_colors = []
    for i, pos in enumerate(classified_positions):
        if pos in total_by_position and total_by_position[pos] > 0:
            positions.append(pos)
            counts.append(total_by_position[pos])
            chart_colors.append(colors[i])
    
    plt.pie(counts, labels=positions, autopct='%1.1f%%', startangle=90, colors=chart_colors)
    plt.title('전체 직급 분포 (담당/책임/수석/기타)')
    plt.axis('equal')
    plt.tight_layout()
...
# 각 열을 조로 인식 (20개 열 = 20개 조)
for j in range(len(df.columns)):  # 열 순회
    team_num = j + 1  # 조 번호 (1부터 시작)
    team_stats[team_num] = {}
    
    for i in range(len(df)):  # 행 순회 (각 조의 멤버들)
        name = df.iloc[i, j]  # i행 j열의 이름
...
# 분류된 직급별 헤더 추가
for idx, position in enumerate(classified_positions):
    ws_stats.cell(row=1, column=idx+2, value=position)

ws_stats.cell(row=1, column=len(classified_positions)+2, value="총원")

# 각 조별 데이터 입력
for team_num in sorted(classified_team_stats.keys()):
    ws_stats.cell(row=team_num+1, column=1, value=f"{team_num}조")
    
    total_count = 0
    for idx, position in enumerate(classified_positions):
        count = classified_team_stats[team_num].get(position, 0)
        ws_stats.cell(row=team_num+1, column=idx+2, value=count)
        total_count += count
    
    ws_stats.cell(row=team_num+1, column=len(classified_positions)+2, value=total_count)

# 전체 합계 행 추가
ws_stats.cell(row=len(classified_team_stats)+2, column=1, value="전체")
for idx, position in enumerate(classified_positions):
    total = sum(team_data.get(position, 0) for team_data in classified_team_stats.values())
    ws_stats.cell(row=len(classified_team_stats)+2, column=idx+2, value=total)

# 전체 총원
grand_total = sum(sum(team_data.values()) for team_data in classified_team_stats.values())
ws_stats.cell(row=len(classified_team_stats)+2, column=len(classified_positions)+2, value=grand_total)

# 차트 생성 및 엑셀에 삽입
chart_files = create_team_charts(classified_team_stats, classified_positions)
